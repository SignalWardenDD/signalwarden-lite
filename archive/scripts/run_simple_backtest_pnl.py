#!/usr/bin/env python3
"""
Простой бэктест с PnL-based системой трейлинга (без зависимости от live системы)
"""
...
def fetch_historical_data(symbol, timeframe, start_date, end_date, limit=1000):
    ...
    while current_ts < end_ts:
        try:
            ...
            current_ts = ohlcv[-1][0] + 1  # Следующий timestamp
            ...
            print(f"    ⚠️ Ошибка загрузки данных для {symbol}: {e}")
            break
...
def generate_signals_for_symbol(symbol, df):
    """Generate signals for a symbol"""
    try:
        # Добавляем индикаторы
        df_with_indicators = add_indicators(df.copy())
        
        # Вычисляем BTC market bias (используем BTC данные если доступны)
        btc_data = None
        if symbol != 'BTC_USDT':
            try:
                btc_data = fetch_historical_data('BTC/USDT', '1h', '2022-01-01', '2024-01-01', 1000)
            ...
            btc_bias = compute_market_bias(df_with_indicators)
        
        # Генерируем сигналы
        signals = generate_signals(df_with_indicators, btc_bias)
        
        return signals
        
    except Exception as e:
        print(f"    ⚠️ Ошибка генерации сигналов для {symbol}: {e}")
        return None

def run_simple_pnl_backtest():
    """Run simple backtest with PnL-based trailing system"""
    
    print("🚀 ПРОСТОЙ БЭКТЕСТ С PnL-based СИСТЕМОЙ ТРЕЙЛИНГА")
    print("=" * 80)
    ...
    for symbol in config['symbols']:
        ...
        try:
            ...
            # Генерируем сигналы
            signals = generate_signals_for_symbol(symbol, df)
