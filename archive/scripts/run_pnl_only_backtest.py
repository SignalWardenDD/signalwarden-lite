#!/usr/bin/env python3
"""
Бектест системы только с PnL-based защитой
Точная копия существующего бектеста, но с PnL-only трейлингом
"""
...
from signalwarden_lite.core.trailing_pnl_only import TrailingConfigPnLOnly
from signalwarden_lite.backtest.engine import run_backtest_one_pnl_only, FeesCfg
...
def _prep(df, cfg, gate):
    """Подготовка сигналов (точная копия оригинального бектеста)"""
...
def main():
    print("🔍 PnL-ONLY БЭКТЕСТ v1.6-TXB")
    print("=" * 60)
    
    # Fixed parameters (точная копия оригинального бектеста)
    ...
    print("🔧 НАСТРОЙКИ PnL-ONLY СИСТЕМЫ:")
    print(f"   💰 Размер позиции: ${cfg['risk']['margin_usdt']} USDT")
    print(f"   🛡️ SL множитель: {cfg['risk']['sl_atr_mult']}x ATR")
    print(f"   📊 PnL-уровни: $0.06→50%, $0.15→60%, $0.25→70%, $0.35→80%")
    print(f"   ❌ R-трейлинг: ОТКЛЮЧЕН")
    print(f"   ❌ Break-Even: ОТКЛЮЧЕН")
    print(f"   ❌ Chandelier: ОТКЛЮЧЕН")
    print()
    ...
    # Process each symbol (точная копия оригинального бектеста)
    rows, all_tr = [], []
    for sym in cfg['symbols']:
        ...
        try:
            ...
            tr_1h = run_backtest_one_pnl_only(sig_1h, sym, cfg['risk']['margin_usdt'], 
        ...
        try:
            ...
            tr_15m = run_backtest_one_pnl_only(sig_15m, sym, cfg['risk']['margin_usdt'], 
                                             cfg['risk']['leverage'], cfg['risk']['sl_atr_mult'], 
                                             trailing, fees)
    ...
    rep.to_csv('backtest_report_PNL_ONLY_v1_6_TXB.csv', index=False)
    
    if all_tr:
        all_trades = pd.concat(all_tr, ignore_index=True)
        all_trades.to_csv('backtest_trades_PNL_ONLY_v1_6_TXB.csv', index=False)
    ...
    if not rep.empty:
        ...
        print(f"\n🎯 ИТОГОВЫЕ РЕЗУЛЬТАТЫ PnL-ONLY СИСТЕМЫ:")
