# Точная копия оригинального бэктеста v1.6-TXB
...
def main():
    print("🔍 ОРИГИНАЛЬНЫЙ БЭКТЕСТ v1.6-TXB")
    print("=" * 60)
    
    # Fixed parameters for original backtest
    ...
    # КРИТИЧЕСКОЕ: Используем ОРИГИНАЛЬНЫЕ настройки для базового бэктеста
    print("🔧 НАСТРОЙКИ ОРИГИНАЛЬНОГО БЭКТЕСТА:")
    
    # Оригинальный R-based трейлинг (как в прибыльном v1.6-TXB)
    original_trailing = {
        'activate_R': 0.35,
        'step_R': 0.20,
        'move_to_be_at_R': 0.35,
        'be_offset_R': 0.08,
        'min_keep_R': 0.15,  # ОРИГИНАЛЬНОЕ значение!
        'chandelier_k_atr': 3.5,
        'symbol_min_keep': None  # БЕЗ индивидуальных порогов в оригинале
    }
    
    # Оригинальные настройки риска
    original_risk = {
        'margin_usdt': 21,  # $21 позиция
        'leverage': 5,
        'sl_atr_mult': 1.5  # 1.5x ATR
    }
    
    print(f"   💰 Размер позиции: ${original_risk['margin_usdt']} USDT")
    print(f"   🛡️ SL множитель: {original_risk['sl_atr_mult']}x ATR")
    print(f"   📊 R-трейлинг: активация {original_trailing['activate_R']}R, шаг {original_trailing['step_R']}R")
    print(f"   🔒 Минимум: {original_trailing['min_keep_R']}R (без индивидуальных порогов)")
    print()
    ...
    trailing = TrailingConfig(**original_trailing)
    ...
    for sym in cfg['symbols']:
        ...
        try:
            ...
            tr_1h = run_backtest_one(sig_1h, sym, original_risk['margin_usdt'], 
                                   original_risk['leverage'], original_risk['sl_atr_mult'], 
                                   trailing, fees)
        ...
        try:
            ...
            tr_15m = run_backtest_one(sig_15m, sym, original_risk['margin_usdt'], 
                                    original_risk['leverage'], original_risk['sl_atr_mult'], 
                                    trailing, fees)
    ...
    rep.to_csv('backtest_report_ORIGINAL_v1_6_TXB.csv', index=False)
    
    if all_tr:
        all_trades = pd.concat(all_tr, ignore_index=True)
        all_trades.to_csv('backtest_trades_ORIGINAL_v1_6_TXB.csv', index=False)
        
    print("\n" + "="*80)
    print("📊 ОРИГИНАЛЬНЫЙ БЭКТЕСТ v1.6-TXB (Точная копия)")
    print("="*80)
    if not rep.empty:
        print(rep.round(2))
