#!/usr/bin/env python3
"""
Отладка live системы - проверка точного состояния и условий входа
"""
...
def debug_specific_pairs():
    """Отладка конкретных пар с сигналами"""
    
    print("🔍 ОТЛАДКА LIVE СИСТЕМЫ - ПРОВЕРКА СИГНАЛОВ")
    print("=" * 80)
    ...
    for symbol, binance_symbol in pairs_to_check:
        ...
        # Создать 15m gate из 1h BTC данных (как в live системе)
        gate_15m_data = []
        for _, row_15m in df_15m.iterrows():
            ts_15m = row_15m['timestamp']
            # Найти ближайший 1h BTC timestamp
            closest_idx = None
            min_diff = float('inf')
            ...
            if closest_idx is not None and min_diff < 3600:  # Максимум 1 час разница
