...
class SignalParams:
    ...
    sg_rsi_bear_max:float=52.0
    sg_rsi_bullcorr_max:float=50.0
    sg_min_natr_bear:float=0.9
    sg_min_natr_bullcorr:float=0.8
    sg_require_close_below_ema20_bullcorr:bool=True
    sg_slope_lookback:int=3
    # optional long filter
...
def generate_signals(df: pd.DataFrame, p: SignalParams, market_gate: pd.DataFrame=None) -> pd.DataFrame:
    ...
    # Raw signal combination
    L = ['sig_long_breakout','sig_long_inside','sig_long_tc','sig_long_sq']
    S = ['sig_short_breakout','sig_short_inside','sig_short_tc','sig_short_sq']
    out['allow_long_raw']  = out[L].any(axis=1)
    out['allow_short_raw'] = out[S].any(axis=1)

    # ADAPTIVE SHORT GUARD - ключевое улучшение v1.6-TXB
    slope = out['ema_slow'] - out['ema_slow'].shift(p.sg_slope_lookback)
    
    # Bear market guard (когда BTC в медвежьей фазе)
    bear_guard = (out['ema_fast'] < out['ema_slow']) & (slope < 0) \
                 & (out['rsi'] <= p.sg_rsi_bear_max) & (out['natr'] >= p.sg_min_natr_bear)
    
    # Bull market correction guard (коррекции в бычьем рынке)
    bullcorr_guard = (out['ema_fast'] < out['ema_slow']) \
                     & (out['rsi'] <= p.sg_rsi_bullcorr_max) & (out['natr'] >= p.sg_min_natr_bullcorr) \
                     & (out['close'] < out['ema20'] if p.sg_require_close_below_ema20_bullcorr else True)

    # Long guard (простой RSI фильтр)
    long_guard = (out['rsi'] >= p.lg_rsi_long_min)
    ...
    def _pick(r, side='long'):
        for key, lab in [('breakout','breakout'),('inside','inside'),('tc','trend_cont'),('sq','squeeze')]:
            col_name = f"sig_{side}_{key if key!='tc' else 'tc'}"
            if r.get(col_name, False):
                entry_col = f"{side}_entry_{'breakout' if key=='breakout' else key}"
                return r.get(entry_col, np.nan), lab
        return np.nan, ''
